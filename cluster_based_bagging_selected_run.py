"""
Cluster-based Bagging on SELECTED features: train on clear clusters, test on ambiguous ones.

This script uses selected features from new_dataset1.csv and:
1. Loads clustering results from outputs/selected_pipeline/kmeans_k=7 and kmeans_k=8
...
Usage: python cluster_based_bagging_selected_run.py
"""
...
def load_cluster_assignments_selected(k):
    """Load cluster assignments from selected_pipeline saved pickle file."""
    pkl_path = Path('outputs') / 'selected_pipeline' / f'kmeans_k_{k}' / 'cluster_labels.pkl'
    if pkl_path.exists():
...
def run():
    out = Path('outputs') / 'cluster_based_bagging_selected'
    out.mkdir(parents=True, exist_ok=True)

    # Load SELECTED FEATURES dataset (from selected_pipeline)
    ...
    # Load clustering results (k=7 and k=8) from selected_pipeline
    print("Loading cluster assignments from selected_pipeline...")
    clusters_k7 = load_cluster_assignments_selected(7)
    clusters_k8 = load_cluster_assignments_selected(8)

    if clusters_k7 is None or clusters_k8 is None:
        print("Clustering files not found in selected_pipeline. Running clustering...")
        ...
        # Save for future use
        Path('outputs/selected_pipeline/kmeans_k_7').mkdir(parents=True, exist_ok=True)
        Path('outputs/selected_pipeline/kmeans_k_8').mkdir(parents=True, exist_ok=True)
        with open(Path('outputs/selected_pipeline/kmeans_k_7/cluster_labels.pkl'), 'wb') as f:
            pickle.dump(clusters_k7, f)
        with open(Path('outputs/selected_pipeline/kmeans_k_8/cluster_labels.pkl'), 'wb') as f:
            pickle.dump(clusters_k8, f)

    # Analyze clusters: find clear vs ambiguous
    print("\nAnalyzing cluster composition from selected_pipeline...")
    ...
    for k, clusters in [(7, clusters_k7), (8, clusters_k8)]:
        ...
        plt.ylim(0, 1)
        plt.ylabel('Значение')
        plt.title(f'Bagging: сравнение метрик (k={k}, Логистическая регрессия, selected features)')
        plt.legend()
        plt.tight_layout()
        ...
        plt.ylim(0, 1)
        plt.ylabel('Значение')
        plt.title(f'Bagging: сравнение метрик (k={k}, Дерево решений, selected features)')
        plt.legend()
        plt.tight_layout()
        ...
        with open(k_out / 'cluster_based_bagging_report.md', 'w', encoding='utf-8') as f:
            f.write(f'# Cluster-Based Bagging на отобранных признаках (K={k})\n\n')
            f.write('Отчёт о сравнении sklearn и custom Bagging на явных и неясных кластерах из selected_pipeline.\n\n')

            f.write('## Стратегия\n\n')
            f.write('- **Данные**: новый датасет с отобранными признаками (new_dataset1.csv)\n')
            f.write('- **Кластеризация**: из outputs/selected_pipeline/kmeans_k_*\n')
            ...
            f.write('\n## Анализ\n\n')
            f.write('Модель была обучена на "явных" кластерах (отобранные признаки) и протестирована на "неясных" кластерах.\n')
            f.write('Это проверяет, насколько хорошо модель на отобранных признаках может классифицировать примеры на границе между фишингом и не-фишингом.\n\n')
    ...
    print(f"\nCluster-based bagging analysis (selected features) complete!")
